...
import { X, Play, Pause, Download, Volume2, VolumeX, Maximize2, FileText, Info, BarChart3, Edit, Check, Search } from 'lucide-react';
...
const TranscriptPlayer: React.FC<TranscriptPlayerProps> = ({ transcript, onClose, onAccept }) => {
  ...
  const [activeTab, setActiveTab] = useState<'player' | 'file' | 'transcription'>('player');
  ...
  const subtitles: SubtitleItem[] = [
    { id: 1, startTime: 0, endTime: 5, text: "Welcome to our transcript service." },
    { id: 2, startTime: 5, endTime: 10, text: "This is a sample subtitle for your media file." },
    { id: 3, startTime: 10, endTime: 15, text: "You can see the synchronized text here." },
    { id: 4, startTime: 15, endTime: 20, text: "This demonstrates the SRT subtitle feature." },
    { id: 5, startTime: 20, endTime: 25, text: "Thank you for using TranscribeAI." }
  ];
  ...
  return (
    <div 
      ref={containerRef}
      className={`fixed inset-0 bg-black z-50 ${fullscreen ? '' : 'bg-opacity-75'} flex items-center justify-center p-6`}
    >
      <div className={`p-6 bg-white rounded-2xl shadow-xl w-full ${fullscreen ? 'h-full' : 'max-w-6xl max-h-[85vh]'} flex flex-col`}>
        {!fullscreen && (
          <div className="flex items-center justify-between p-6 border-b border-gray-200 flex-shrink-0">
            <div>
              <h2 className="text-2xl font-bold text-gray-900">{transcript.fileName}</h2>
              <p className="text-gray-600">Playing with synchronized subtitles</p>
            </div>
        ...
        <div className="flex-1 overflow-hidden">
          ...
          <div className={`flex ${fullscreen ? 'h-full' : 'h-96'} lg:h-[500px] p-4`}>
          {/* Media Player */}
            <div className={`${fullscreen ? 'w-full' : 'w-1/2'} bg-black flex items-center justify-center relative`}>
            {isVideo ? (
              <video
              ...
              <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-blue-900 to-purple-900">
                <div className="text-center">
                  ...
                  <h3 className="text-2xl font-bold text-white mb-2">{transcript.fileName}</h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="text-gray-900 bg-gray-50 p-3 rounded-lg">{transcript.fileName}</p>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">File Type</label>
                        <p className="text-gray-900 bg-gray-50 p-3 rounded-lg capitalize">{transcript.fileType}</p>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">File Size</label>
                        <p className="text-gray-900 bg-gray-50 p-3 rounded-lg">{(Math.random() * 50 + 10).toFixed(2)} MB</p>
                      </div>
                    </div>
                    <div className="space-y-4">
                      <div>
                      ...
                      <div>
                        ...
                        <p className="text-gray-900 bg-gray-50 p-3 rounded-lg">{formatTime(duration)}</p>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="text-gray-900 bg-gray-50 p-3 rounded-lg">{(Math.random() * 10 + 2).toFixed(1)} minutes</p>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Accuracy Score</label>
                        <div className="bg-gray-50 p-3 rounded-lg">
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-gray-900">{(Math.random() * 20 + 80).toFixed(1)}%</span>
                            <span className="text-sm text-gray-500">High</span>
                          </div>
                          <div className="w-full bg-gray-200 rounded-full h-2">
                            <div className="bg-green-500 h-2 rounded-full" style={{ width: `${Math.random() * 20 + 80}%` }}></div>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="bg-gray-50 p-3 rounded-lg">
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-gray-900">{(Math.random() * 15 + 85).toFixed(1)}%</span>
                            <span className="text-sm text-gray-500">Very High</span>
                          </div>
                          <div className="w-full bg-gray-200 rounded-full h-2">
                            <div className="bg-blue-500 h-2 rounded-full" style={{ width: `${Math.random() * 15 + 85}%` }}></div>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-green-50 to-emerald-50">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      ...
                      <div>
                        <h3 className="text-xl font-semibold text-gray-900">Accepted Transcript</h3>
                        <p className="text-sm text-gray-600 mt-1">This transcript has been accepted and is ready for use.</p>
                      </div>
                    </div>
                ...
                <div className="p-6">
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="flex items-center space-x-2 text-sm text-gray-500">
                      <span>{filteredSubtitles.length}/{subtitles.length} segments</span>
                      <span>•</span>
        ...
        {activeTab === 'player' && !transcript.accepted && (
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex-1 flex items-center space-x-4">
                <span className="text-sm font-medium text-gray-700 w-16 text-center">{formatTime(currentTime)}</span>
                ...
                <span className="text-sm font-medium text-gray-700 w-16 text-center">{formatTime(duration)}</span>
            </div>
              ...
              <input
                ...
                onChange={handleVolumeChange}
                    ...
                    style={{
                      background: `linear-gradient(to right, #3b82f6 0%, #3b82f6 ${(muted ? 0 : volume) * 100}%, #e5e7eb ${(muted ? 0 : volume) * 100}%, #e5e7eb 100%)`
                    }}
              />
                </div>
            </div>
