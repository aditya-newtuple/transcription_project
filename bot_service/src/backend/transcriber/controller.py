...
class TranscriberRestController:
    ...
    @staticmethod
    def _require_transcriber_from_state(req: Request) -> TranscriberServiceManager:
    ...
    def prepare(self, app: APIRouter) -> None:
        ...
        @app.post(
            ...
            tags=["transcriber/testing"],
            response_model=BulkTranscriptionResponse
        )
        async def transcribe(
            files: List[UploadFile] = File(...),
            transcriber: TranscriberDep = None,
            ...
            for file in files:
                ...
                if file_ext not in self.ALLOWED_EXTENSIONS:
                    logger.warning("Unsupported file format attempted", extra={"file_name": file.filename})
                    raise HTTPException(
                        status_code=status.HTTP_400_BAD_REQUEST,
